...
import com.snowplowanalytics.snowplow.postgres.loader.shredding
import com.snowplowanalytics.snowplow.postgres.loader.shredding.{Type, Entity, transform}
import com.snowplowanalytics.snowplow.postgres.loader.storage.definitions.metaColumns
...
object sql {
   ...
   */
  def createTable(schema: String, entity: shredding.Entity, schemaList: SchemaList, meta: Boolean): Fragment = {
  ...
  def buildColumn(varcharSize: Int, pair: (Pointer.SchemaPointer, Schema)): Column =
    pair match {
      case (pointer, properties) =>
        ...
        Column(columnName, dataType, shredding.schema.canBeNull(properties))
    }
