...
import com.snowplowanalytics.snowplow.postgres.loader.streaming.source.Data
  ...
  "goodSink" should {
    "sink a single good event" >> {
      ...
      val action = for {
        ...
        _ <- stream.through(sink.goodSink(xa, logger, Schema, state, igluClient)).compile.drain.action
