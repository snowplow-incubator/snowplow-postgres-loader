...
import com.snowplowanalytics.snowplow.postgres.config.LoaderConfig.{Monitoring, Source}

object KinesisEnv {

  def create[F[_]: ConcurrentEffect: ContextShift: Timer](blocker: Blocker, config: Source.Kinesis, metrics: Monitoring.Metrics): Resource[F, Environment[F, CommittableRecord]] =
    for {
  ...
  private def getPayload(record: CommittableRecord): Either[String, String] =
    Either.catchNonFatal(StandardCharsets.UTF_8.decode(record.record.data()).toString)
      .leftMap(_ => StandardCharsets.UTF_8.decode(Base64.getEncoder.encode(record.record.data())).toString)
